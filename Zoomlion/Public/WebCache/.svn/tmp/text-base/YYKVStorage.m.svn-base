...
//    printf("database is lock now,can not write/read.\n");  //每次执行一次回调函数打印一次该信息
...
//        sqlite3_busy_handler(_db,callback_db,(void *)_db);
...
- (instancetype)initWithPath:(NSString *)path type:(YYKVStorageType)type {
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        // db file may broken...
        [self _dbClose];
//        [self _reset]; // rebuild    //王li，这里是修改后的，当出现多线程加载时，报错误database is locked时候，这里就reset了，当注释掉后就不会出现数据丢失了。
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
